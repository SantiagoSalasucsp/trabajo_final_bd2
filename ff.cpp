...
struct Tabla {
    ...
    virtual size_t hash_func(const string& clave) = 0;
    virtual void insertar(Enlace* enlace) = 0;
    virtual Enlace* buscar(const string& clave) = 0;
...
private:
    vector<list<EnlaceInteger*>> tabla;
    size_t tam;
...
public:
    ...
    size_t hash_func(const string& clave) override {
        size_t hash = 0;
        for (char c : clave) {
    ...
    void insertar(Enlace* enlace) override {
        EnlaceInteger* enlace_int = dynamic_cast<EnlaceInteger*>(enlace);
        if (!enlace_int) return;

        size_t index = hash_func(enlace_int->);
        tabla[index].push_back(enlace_int);
    }

    Enlace* buscar(const string& clave) override {
        size_t index = hash_func(clave);
        
        for (auto* enlace : tabla[index]) {
            if (enlace->name == clave) {
                return enlace;
            }
        }
...
// Tabla Hash para DECIMAL (similar a TablaHashInteger)
class TablaHashDecimal : public Tabla {
private:
    vector<list<EnlaceDecimal*>> tabla;
    size_t tam;
...
public:
    ...
    void insertar(Enlace* enlace) override {
        EnlaceDecimal* enlace_decimal = dynamic_cast<EnlaceDecimal*>(enlace);
        if (!enlace_decimal) return;

        size_t index = hash_func(enlace_decimal->name);
        tabla[index].push_back(enlace_decimal);
    }
        ...
        while (!obj->tipos.empty()) {
            ...
            if (sector_actual->tamano >= tipo_actual->tam) {
                ...
                if (varchar_actual) {
                    ...
                    //obj->reserva.push_back(varchar_actual);
